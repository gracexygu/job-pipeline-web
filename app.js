...
function startColumnResize(event) {
  event.preventDefault();
  const column = state.columns.find(item => item.id === Number(event.currentTarget.dataset.resizeColumn));
  if (!column) return;
  const startX = event.clientX;
  const startWidth = column.width;
  const move = moveEvent => {
    column.width = Math.max(90, Math.min(600, Math.round(startWidth + moveEvent.clientX - startX)));
    const header = document.querySelector(`th[data-column-id="${column.id}"]`);
    if (header) header.style.width = `${column.width}px`;
    document.querySelector(".data-grid").style.width = `max(100%, ${state.columns.reduce((sum, item) => sum + item.width, 44)}px)`;
  };
  const up = async () => {
    document.removeEventListener("pointermove", move);
    document.removeEventListener("pointerup", up);
    try {
      await fetch(`./api/table-columns/${column.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ patch: { width: column.width }, expectedRevision: column.local_revision }) }).then(assertOk);
    } catch (error) { showError(error); await load(); }
  };
