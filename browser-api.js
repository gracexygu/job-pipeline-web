...
function newPosition(data, input) {
  ...
  const item = {
    ...
    deadline: input.deadline || "", category: input.category || "", application_rule: input.application_rule || "", job_natures: [], formal_validity: "不确定",
...
function updatePosition(data, id, patch, expectedRevision) {
  ...
  const allowed = ["company", "role_name", "jd", "official_url", "deadline", "assessment_start", "assessment_content", "category", "application_rule", "formal_validity", "recommendation", "final_result", "resume_version", "stage_notes", "next_action", "next_action_due"];
