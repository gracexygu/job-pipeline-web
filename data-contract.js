...
export const DEFAULT_COLUMNS = [
  ...
  ["application_rule", "投递规则", "textarea", "application_rule", 240, []],
  ["next_action", "Agent 建议", "textarea", "next_action", 240, []],
...
export function migrateWebState(value) {
  ...
  const knownFields = new Set(state.columns.map(column => column.source_field));
  columnsAt(timestamp).filter(column => ["application_rule", "next_action"].includes(column.source_field) && !knownFields.has(column.source_field)).forEach(column => {
    column.id = Math.max(0, ...state.columns.map(item => Number(item.id) || 0)) + 1;
    column.position = state.columns.length;
